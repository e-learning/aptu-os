...
void heap_push(struct heap *restrict h, type value)
{
	...
	{
		parent = (index - 1) >> 1;
        if (h->cmp(h->data[parent], value)) break;
...
void heap_pop(struct heap *restrict h)
{
	...
	for(index = 0; 1; index = swap)
	{
		...
		other = swap + 1;
        if ((other < h->count) && h->cmp(h->data[other], h->data[swap])) swap = other;
        if (h->cmp(temp, h->data[swap])) break; // If the bigger child is less than or equal to its parent, the heap is reordered
