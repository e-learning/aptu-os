...
file_system::file_system(const char *root)
    : root(root),
      config(get_config()),
      BLOCK_DATA_SIZE(config.block_size - sizeof(block_record)),
...
void file_system::format() 
{
    ...
    int rd_size = sizeof(file_record);
...
void file_system::rm(file_record const& fd) 
{
    if (fd.directory) 
		{
        if (fd.first_child != -1) 
            ...
            do 
						{
                file_record c = read_descriptor(next_child);
                rm(c);
...
vector<file_record> file_system::list(file_record const& directory) const 
{
    vector<file_record> fds;
...
file_record file_system::init_descriptor(const string & name, bool directory) 
{
    file_record dd;
    dd.id               = -1;
...
void file_system::clear_descriptor(file_record const& fd) 
{
    if (fd.first_block != -1) 
		{
        block_record bd = read_descriptor<block_record>(fd.first_block);
        while (bd.next != -1) 
				{
            block_map[bd.id] = false;
            bd = read_descriptor<block_record>(bd.next);
        }
        block_map[bd.id] = false;
    }
}
...
{
    file_record fd = read_descriptor(id);

    if (fd.prev_file == -1)
		{       
			if (fd.next_file == -1) 
			{
            file_record parent = read_descriptor(fd.parent_file);
			...
			{ 
            file_record parent = read_descriptor(fd.parent_file);
            file_record next = read_descriptor(fd.next_file);
        ...
        if (fd.next_file == -1) 
				{
            file_record prev = read_descriptor(fd.prev_file);
            prev.next_file = -1;
...
file_record file_system::find_descriptor(file_record & directory, const string & descriptor_name, bool create, bool is_directory) 
{
    if (directory.first_child == -1)
		{
        if (create) 
				{
            file_record new_descriptor = init_descriptor(descriptor_name, is_directory);
		...
		{                                    
        file_record current = read_descriptor(directory.first_child);
        ...
        if (create) 
				{
					file_record new_file = init_descriptor(descriptor_name, is_directory);
...
file_record file_system::find_descriptor(const char *destination,  bool create, bool is_directory) 
{
    vector<string> path = split(string(destination), '/');
    string descriptor_name = path.back(); path.pop_back();
    if (descriptor_name == "")
        return root_d;
    file_record directory = find_directory(path, create);
    return find_descriptor(directory,descriptor_name,create, is_directory);
}

file_record file_system::find_directory(const char *destination, bool create) 
{
    return find_directory(split(destination, '/'), create);
}

file_record file_system::find_directory(const vector<string> & path, bool create) 
{
    if (path.size() < 1 || path[0] != "")
        throw "Path should start with /";

    file_record current = root_d;
    ...
    for (auto it = path.begin() + 1; it != path.end(); ++it) 
		{
			if (current.first_child == -1) 	            
				if (create) 
				{
					file_record new_directory = init_descriptor(*it);
...
void file_system::read_data(const file_record & fd, std::ostream & destination_stream) 
{
    ...
    while (next != -1)
        ...
        block_record block_d;
        block.read((char *) &block_d, sizeof(block_record));
...
void file_system::write_data(file_record & fd, std::istream & source_stream) 
{
    clear_descriptor(fd);
    char * buf = new char[BLOCK_DATA_SIZE];
    ...
    while (source_stream)
        ...
        block.write((char *) &bd, sizeof(block_record));
...
void file_system::read_block_map()
{
    ...
    block.seekg(sizeof(file_record));
...
void file_system::write_block_map() 
{
    ...
    block.write((char *)&root_d, sizeof(file_record));
    if (!block)
        throw "Can't open block";
    block.seekp(sizeof(file_record));
    for (int i = 0; i < config.block_number;) 
		{
			char byte = 0;
			for (int b = 0; b < 8 && i < config.block_number; b++,i++)
...
void file_system::copy(const file_record &from, file_record &to) 
{
	...
	if (from.directory && to.directory) 
	{
		if (from.first_child != -1) 
		{
			...
			do
			{
				file_record child = read_descriptor(next_child);
				file_record new_d = find_descriptor(to, child.name, true, child.directory);
				copy(child, new_d);
	...
	{
		file_record new_file = find_descriptor(to, from.name, true, false);
		copy(from, new_file);
	}
	else if (!from.directory && !to.directory) 
	{
		...
		do 
		{
			...
			block_record new_block_d = block_d;
			...
			new_block.write((char *)&new_block_d, sizeof(block_record));
...
void file_system::move(const file_record & source_d, file_record & destination_d) 
{
	if (source_d.directory)
	{
		if (destination_d.directory)
		{
			...
			{
				file_record first_child = read_descriptor(source_d.first_child);
				...
				{
					file_record last_child = read_descriptor(destination_d.first_child);
