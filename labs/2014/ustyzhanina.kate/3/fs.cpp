...
pair<string, size_t> FileSystem:: find_directory_not(string instr, bool up_to_end)
{
	vector <string> parts  = utils.split(instr, '/');
...
queue <pair<string, size_t> > delete_elem(string elem, queue <pair<string, size_t> > list )
{
	...
	queue <pair<string, size_t> > rez_list;

	for (int i = 0; i != list_size; ++i)
	{
		if (list.front().first != elem)
		{
			rez_list.push(list.front());
		}
...
void FileSystem::rm_one_file(DirectoryD source)
{
	...
	for (size_t i = 0; i != list_size; ++i)
	{
		...
		if (ident == FILE)
		{
			...
			source.files = delete_elem(cur_file.name, source.files);
			fill_free_block(cur_file.blocks);
			fill_free_block(source.blocks);
			//write_directory(source, source.blocks[0]);
			list.pop();
		}
		else
		{
			DirectoryD new_source = read_directory(list.front().second);
			fill_free_block(new_source.blocks);
			source.files = delete_elem(new_source.name, source.files);
			list.pop();
			rm_one_file(new_source);
		}
	}

	fill_free_block(source.blocks);
}
void FileSystem::rm_file(string from)
{
	read_config();
	read_meta_data();

	if (from == "/")
		rm_one_file(read_directory(meta_blocks_count));
	else
	{
		DirectoryD source = read_directory(find_directory(from, false).second);
		pair<string, size_t> new_elem = find_directory(from, true);
		...
		{
			cur_dir = read_directory(new_elem.second);
			source.files = delete_elem(cur_dir.name, source.files);
			rm_one_file(cur_dir);
		}

		fill_free_block(cur_dir.blocks);
		fill_free_block(source.blocks);
		source.time = time(0);
		write_directory(source, source.blocks[0]);
	}
...
void FileSystem::move(string  const source, string const to)
{
	...
	DirectoryD to_dir = read_directory(find_directory(to, false).second);
	bool find = false;
	size_t try_to_dir = find_directory_not(to, true).second;

	if (try_to_dir != -1)
	{
		ifstream file(utils.path_append(root_dirname, to_string(try_to_dir)));
		size_t m_file;
		file.read((char *)&m_file, sizeof(size_t));

		if (m_file == DIR)
		{
			to_dir = read_directory(try_to_dir);
			find  = true;
		}
	}
	...
	if (find)
	{
		if (check_exist_file(new_elem.first, to_dir.files))
			throw runtime_error("this file " + to + "/" + new_elem.first + " already exist in filesystem");

		to_dir.files.push(make_pair(new_elem.first, new_elem.second));
	}
	else
	{
		if (check_exist_file(to_path[to_path.size() - 1], to_dir.files))
			throw runtime_error("this file " + to + " already exist in filesystem");

		ifstream file(utils.path_append(root_dirname, to_string(new_elem.second)));
		size_t m_file, data_size, next_block;
		time_t time_m;
		file.read((char *)&m_file, sizeof(size_t));
		string file_name = utils.read_string(file);
		file.read((char *)&time_m, sizeof(time_t));
		file.read((char *)&data_size, sizeof(size_t));
		file.read((char *)&next_block, sizeof(size_t));
		file.close();
		ofstream file_out(utils.path_append(root_dirname, to_string(new_elem.second)));
		time_m = time(0);
		file_name = to_path[to_path.size() - 1];
		file_out.write((char *)&FILE, sizeof(size_t));
		utils.write_string(file_out, file_name);
		file_out.write((char *)&time_m, sizeof(time_t));
		file_out.write((char *)&data_size, sizeof(size_t));
		file_out.write((char *)&next_block, sizeof(size_t));
		file_out.close();
		to_dir.files.push(make_pair(to_path[to_path.size() - 1], new_elem.second));
	}

	to_dir.time = time(0);
	fill_free_block(to_dir.blocks);
	write_directory(to_dir, to_dir.blocks[0]);
	DirectoryD source_dir  = read_directory(find_directory(source, false).second);
	source_dir.files = delete_elem(new_elem.first , source_dir.files);
	source_dir.time = time(0);
	fill_free_block(source_dir.blocks);
	write_directory(source_dir, source_dir.blocks[0]);
	write_meta();
}
...
void FileSystem::copy(string  source, string to)
{
	...
	if (ident == FILE)
	{
		...
		else
		{
			...
			ifstream file(utils.path_append(root_dirname, to_string(new_block)));
			size_t m_file, data_size, next_block;
			time_t time_m;
			file.read((char *)&m_file, sizeof(size_t));
			string file_name = utils.read_string(file);
			...
			file.read((char *)&next_block, sizeof(size_t));
			file.close();
			ofstream file_out(utils.path_append(root_dirname, to_string(new_block)));
			time_m = time(0);
			file_name = to_path[to_path.size() - 1];
			file_out.write((char *)&FILE, sizeof(size_t));
			utils.write_string(file_out, file_name);
			...
			file_out.write((char *)&next_block, sizeof(size_t));
			file_out.close();
			to_dir.files.push(make_pair(to_path[to_path.size() - 1], new_block));
		}
	}
	else
	{
...
void FileSystem::ls(string path)
{
	...
	find_directory_not(path, true);

	if ((cur_ptr.second == -1)  || (find_directory_not(path, true).second == -1))
		throw runtime_error("no such directiry or file");
