...
int main(int argc, char **argv)
{
	...
	else
	{
		...
		else
		{
			...
			WriteStruct(&prev,file_inode.nb);
			WriteStruct(&nextbr,file_inode.pb);
		}
	}	
	time(&file_inode.lm_time);	
	file_inode.nb = NULL;
	...
	file_inode.par = par_dir.first_block_addr;
	int lf;
	if (par_dir_dir.ff==NULL)
	{
		lf=par_dir.first_block_addr;
		par_dir_dir.ff = file_id_block;
